...
def parse_film(link):
    try:
        ...
        except: print datetime.strftime(datetime.now(), "%Y.%m.%d %H:%M:%S"), "[ERROR]: ", link, h1, sys.exc_info()[0]
        ...
        print datetime.strftime(datetime.now(), "%Y.%m.%d %H:%M:%S"), "[ERROR]: ", link, h1, sys.exc_info()[0]
        return None
...
# Открываем базу данных    
    ...
    while i <= namespace.end:
        ...
        print datetime.strftime(datetime.now(), "%Y.%m.%d %H:%M:%S"), "[INFO ]: URL:", url
        cnt = get_content(url)
